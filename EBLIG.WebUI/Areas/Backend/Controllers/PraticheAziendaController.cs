...
namespace EBLIG.WebUI.Areas.Backend.Controllers
{
    ...
    public class PraticheAziendaController : BaseController
    {
        ...
        {
            var _query = unitOfWork.PraticheRegionaliImpreseRepository.Get(RicercaFilter(model)).AsQueryable()
        ...
        public ActionResult CreateNuovaRichiesta(PraticheAziendaNuovaRichiesta model)
        {
            try
            {
                ...
                if (IsUserConsulenteCs)
                {
                    ...
                    if (_valid.Count() > 0)
                    {
                        throw new Exception($"Attenzione, per inserire una richiesta, completa prima anagrafica del azienda \"{_result.RagioneSociale}\"");
                    }
                }
        ...
        public ActionResult ApriRichiesta(int id)
        {
            try
            {
                ...
                //eblig po modificare le richieste in bozza e revisione
        ...
        {
            var tiporichiesta = unitOfWork.TipoRichiestaRepository.Get(x => x.TipoRichiestaId == tipoRichiestaId).FirstOrDefault();
            ...
            model.AbilitatoNuovaRichiesta = tiporichiesta.AbilitatoNuovaRichiesta.GetValueOrDefault();

            if (statoId == (int)EbligEnums.StatoPratica.Revisione)
            {
                model.AbilitatoNuovaRichiesta = true;
            }

            if (!tiporichiesta.AbilitatoNuovaRichiesta.GetValueOrDefault() && IsUserAdmin)
            {
                model.AbilitatoNuovaRichiesta = true;
            }
        ...
        public IEnumerable<VisualizzaBudgetViewModel> GetImportiRichiestaPerTipoRichiesta()
        {
        ...
        public ActionResult GetImportoTotaleRimborsatoFormazioneAggiornamentoProfessionale(string importoAccettato, int tipoRichiestaId, int motivazioneRichiestaId)
        {
            decimal.TryParse(importoAccettato, out decimal _importoAccettato);
            var _importoRimborsato = PraticheAziendaUtility.GetImportoTotaleRimborsatoFormazioneAggiornamentoProfessionale(_importoAccettato, motivazioneRichiestaId);
            return GetImportoCalcolatiResult(tipoRichiestaId, _importoRimborsato);
        }
        ...
        public ActionResult GetImportoIncrementoMantenimentoOccupazionImpreseTotale(string importoTotale, int tipoRichiestaId)
        {
            decimal.TryParse(importoTotale, out decimal _importoTotale);

            return GetImportoCalcolatiResult(tipoRichiestaId, _importoTotale);
        }
