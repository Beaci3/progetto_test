...
namespace EBLIG.WebUI.Areas.Admin.Controllers
{
    ...
    public class TipoRichiesteController : BaseController
    {
        ...
        private Expression<Func<TipoRichiesta, bool>> RicercaFilter(TipoRichiesteRicercaModel model)
        {
            ...
            && (model.Desrcizione != null && model.Desrcizione != ""? x.Descrizione.Contains(model.Desrcizione) : true)
        ...
        public ActionResult Duplica(TipoRichiesta model)
        {
            try
            {
                ...
                var t = unitOfWork.TipoRichiestaRepository.Get(x => x.IsTipoRichiestaDipendente == tipoRichiesta.IsTipoRichiestaDipendente 
                && x.Descrizione == tipoRichiesta.Descrizione 
                && x.Anno == model.Anno);
                if (t.Count() > 0)
                {
